...
function renderRecords() {
  ...
  patients.forEach(p => {
    ...
    allFields.forEach(f => {
      ...
      if (f.id === "department") {
        if (p.department === "Tenants" && p.tenantName) {
          cell.textContent = `${p.department} - ${p.tenantName}`;
        } else {
          cell.textContent = p.department || "";
        }
      } else if (f.id === "sex") {
        cell.textContent = p.sex === "M" ? "Male" : p.sex === "F" ? "Female" : "";
      } else if (f.id === "medication" && p.medication) {
        cell.textContent = p.medication;
      } else if (f.id === "patientName") {
        // Combine lastName, firstName, and optional middleName (or initial)
        const last = p.lastName || "";
        const first = p.firstName || "";
        const middle = p.middleName ? ` ${p.middleName}` : "";
        cell.textContent = `${last}, ${first}${middle}`.trim();
      } else {
...
function showFullHistory(patientId) {
  ...
  title.textContent = `Full History for ${patient.lastName || ""}, ${patient.firstName || ""}${patient.middleName ? " " + patient.middleName : ""}`.trim();
